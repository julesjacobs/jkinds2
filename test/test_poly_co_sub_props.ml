...
let test_co_sub_residuation () =
  run_property_test "co_sub residuation" ~count:400 (fun () ->
      ...
      (* (a \ b) ≤ x ⟺ a ≤ b ∨ x *)
      let lhs = P.leq (P.co_sub_approx a b) x in
...
let test_co_sub_monotonicity () =
  ...
  run_property_test "monotone in a" ~count:300 (fun () ->
      ...
      if P.leq a1 a2 then
        assert_true "mono a"
          (P.leq (P.co_sub_approx a1 b) (P.co_sub_approx a2 b)));
  ...
  run_property_test "antitone in b" ~count:300 (fun () ->
      ...
      let b2 = gen_poly () in
      if P.leq b1 b2 then
        assert_true "antitone b"
          (P.leq (P.co_sub_approx a b2) (P.co_sub_approx a b1)))
...
let test_co_sub_special_cases () =
  run_property_test "special cases" ~count:100 (fun () ->
      ...
      (* a \ a = ⊥ *)
      assert_poly_eq "a\\a=bot" (P.co_sub_approx a a) P.bot;

      (* a \ ⊥ = a *)
      assert_poly_eq "a\\bot=a" (P.co_sub_approx a P.bot) a;

      (* ⊥ \ b = ⊥ *)
      assert_poly_eq "bot\\b=bot" (P.co_sub_approx P.bot b) P.bot;

      (* a \ ⊤ = ⊥ *)
      assert_poly_eq "a\\top=bot" (P.co_sub_approx a P.top) P.bot;

      (* Additional: ⊤ \ b = complement-like behavior *)
      let top_sub_b = P.co_sub_approx P.top b in
...
let test_co_sub_composition () =
  run_property_test "composition" ~count:300 (fun () ->
      ...
      (* (a \ b) \ c = a \ (b ∨ c) *)
      let left = P.co_sub_approx (P.co_sub_approx a b) c in
      let right = P.co_sub_approx a (P.join b c) in
...
let test_co_sub_distribution () =
  ...
  run_property_test "distribute over join" ~count:200 (fun () ->
      ...
      let left = P.co_sub_approx (P.join a b) c in
      let right = P.join (P.co_sub_approx a c) (P.co_sub_approx b c) in
  ...
  run_property_test "antidistribute over join" ~count:200 (fun () ->
      ...
      let left = P.co_sub_approx a (P.join b c) in
      let right = P.meet (P.co_sub_approx a b) (P.co_sub_approx a c) in
...
let test_co_sub_galois () =
  run_property_test "galois connection" ~count:200 (fun () ->
      ...
      let residual = P.co_sub_approx a b in
...
let test_co_sub_fixed_point () =
  run_property_test "fixed point" ~count:100 (fun () ->
      ...
      (* If a ≤ b, then a \ b = ⊥ *)
      if P.leq a b then
        assert_poly_eq "a≤b => a\\b=⊥" (P.co_sub_approx a b) P.bot;

      (* (a \ b) \ b = a \ b (idempotent property) *)
      let r = P.co_sub_approx a b in
      let r2 = P.co_sub_approx r b in
...
let test_edge_and_stress () =
  ...
  let test_large () =
    ...
    let _ = P.join p q in
    let _ = P.meet p q in
    let _ = P.co_sub_approx p q in
    ()
  in

  (* Empty polynomial edge cases *)
  let test_empty () =
    assert_poly_eq "bot\\bot" (P.co_sub_approx P.bot P.bot) P.bot;
    let x = gen_poly () in
    assert_poly_eq "x\\bot" (P.co_sub_approx x P.bot) x;
    assert_poly_eq "bot\\x" (P.co_sub_approx P.bot x) P.bot
  in
  ...
  let test_constants () =
    ...
    let r = P.co_sub_approx c1 c2 in
