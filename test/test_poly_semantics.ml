...
let test_substitution_semantics () =
  run_property_test "substitution semantics" ~count:300 (fun () ->
      ...
      let rho' v =
        match P.VarMap.find_opt v !subs with
        | None -> rho v
...
let test_order_properties () =
  ...
  run_property_test "antisymmetry" ~count:50 (fun () ->
      let p = gen_poly () in
      let q = gen_poly () in
      if P.leq p q && P.leq q p then assert_poly_eq "antisymmetric" p q);
...
let test_specific_semantics () =
  ...
  let p =
    P.join (P.meet x (P.const (encode 2 0))) (P.meet y (P.const (encode 1 1)))
  in
  let rho = fixed_env (encode 2 1) (encode 0 1) C.bot C.bot in
  let result = P.eval rho p in
  let expected =
    C.join (C.meet (encode 2 1) (encode 2 0)) (C.meet (encode 0 1) (encode 1 1))
  in
