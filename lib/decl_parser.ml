...
let parse_program_exn (s : string) : decl_item list =
  ...
  let parse_line (line : string) : decl_item =
    ...
    let sep_idx, abstract =
      ...
      | Some i, None -> (i, false)
      | None, Some i -> (i, true)
      | Some _, Some _ -> raise (Parse_error "declaration must use exactly one of '=' or ':'")
      | None, None -> raise (Parse_error "missing '=' or ':' in declaration")
    in
    let lhs = String.trim (String.sub rest 0 sep_idx) in
    let rhs = String.trim (String.sub rest (sep_idx + 1) (String.length rest - sep_idx - 1)) in
  ...
  let _ =
    List.fold_left
      (fun acc it -> if NameMap.mem it.name acc then raise (Parse_error ("duplicate declaration: " ^ it.name)) else NameMap.add it.name () acc)
      NameMap.empty items
  in
  items