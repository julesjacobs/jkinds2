...
(* Merge Modality_poly into this module by specializing lattice polynomials to Coeff. *)
...
let pp (m : t) : string =
  ...
  if P.equal m P.bot then "⊥"
    ...
    let is_id = match ts with [ (s,c) ] -> P.VarSet.is_empty s && Coeff.equal c Coeff.top | _ -> false in
    if is_id then "⊤"
    else
      let pp_vars vars =
