...
struct
  ...
  module LSolver = Udd.Make (Lat) (RigidName)
  ...
  let make_solver (env : env) : solver =
    (* Infer8: hard-code best option from experiments: enqueue LFPs in
       constr_kind, while keeping eager solve in kind_of. *)
    let enqueue_lfp_enabled = true in
    ...
    let rec kind_of (t : ty) : kind =
      ...
      | None ->
        ...
        (* Eagerly solve LFPs here to avoid correctness issues *)
    ...
    and constr_kind c =
      ...
      | None -> (
        ...
        | Ty { args; kind; abstract } ->
          ...
          else if enqueue_lfp_enabled then (
  ...
  let make_normalizer (env : env) : solver =
    (* Define all the trivial ops *)
    let const l = LSolver.const l in
    let join ks = List.fold_left LSolver.join LSolver.bot ks in
    let modality l k = LSolver.meet (LSolver.const l) k in
        ...
        | Ty { args; kind; abstract } ->
          if abstract then
            ...
            if List.length coeffs <> List.length coeffs' then
              failwith
                (Printf.sprintf
                   "jkind_solver: coeffs mismatch for constr %s (length %d vs \
                    %d)"
                   (Constr.to_string c) (List.length coeffs)
                   (List.length coeffs'));
            LSolver.enqueue_lfp base base';
            List.iter2
              (fun coeff coeff' -> LSolver.enqueue_lfp coeff coeff')
